...
namespace EasyBuy.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult TrangChu(string? search, int? cate, int? brandId, decimal? minPrice, decimal? maxPrice, string? sortBy)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(sortBy))
                {
                    ...
                    {
                        case "banchay":
                            products = products.OrderByDescending(p => p.QuantitySold); // cột QuantitySold phải có
                            break;
                        case "gia_tang":
